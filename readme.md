...
> **Why this stack?**  
> Starting with vanilla HTML, CSS, and JavaScript keeps the frontend simple, fast, and easy to deploy. It avoids framework overhead while we focus on core features. The API layer in FastAPI provides a fast, typed backend that can easily integrate with any external services.
...
|-------|----------------|-----|
| **Front‑end** | HTML, CSS, JavaScript | Simple, universal, no-build-step development. |
| **Auth** | Auth0 or similar JWT-based service | Secure login for admin/back‑office, optional customer auth. |
...
| **CI/CD** | GitHub Actions → Docker Hub → Render / Netlify | Automated builds, linting, testing, and deployment. |
...
| **Responsive Layout** | Mobile‑first design using CSS media queries. | ★★ |
...
# Frontend dev server
cd frontend
# Open index.html in your browser. For live-reloading, use a tool like VS Code's "Live Server" extension.
...
> **Environment Variables** – copy `backend/.env.example` to `backend/.env` and fill in API keys, DB credentials, Stripe secrets.
...
| Platform | How |
|----------|-----|
| **Frontend** | Netlify / GitHub Pages – deploy `frontend/` as a static site. |
| **Backend** | Render or Fly.io – Docker container from `backend/Dockerfile`. |
| **Database** | Render Postgres (or self‑hosted). |
| **Domain** | dragriver.ca → Point to Netlify & Render services. |
...
| Sprint | Goals |
|--------|-------|
| **Sprint 1** | Set up repo, CI/CD, Docker environment, basic HTML/CSS/JS scaffold. |
| **Sprint 2** | Global layout + navigation; CSS styling; SEO basics. |
